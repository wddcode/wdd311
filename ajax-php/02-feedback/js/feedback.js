		...
		$.ajax({
			...
			success: function(data) {
				...
				for(k in validation) {
					...
					// sorry - very bad handling here... should be rewritten
					...
					if (v.status != true) {
						...
						if(k == 'email') {
							...
							$('.help-inline', $('#inputEmail').parents('.control-group')).html('');
						}
						if(k == 'name') {
							$('#inputName').parents('.control-group').addClass('success');
							$('.help-inline', $('#inputName').parents('.control-group')).html('');
						}
						if(k == 'comment') {
							$('#inputComment').parents('.control-group').addClass('success');
							$('.help-inline', $('#inputComment').parents('.control-group')).html('');
						}
					}
